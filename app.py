...
if not st.session_state.show_results:
    ...
    # 수정된 부분: selected_category에 따라 필터링하는 로직
    if selected_category == "연애":
        filter_category = "관계 고민_연애"
        # '관계 고민_연애' 카테고리가 있는 스프레드와 '관계 고민' 카테고리가 있는 스프레드 모두 포함
        valid_spreads = [
            name for name in all_spread_display_names
            if "관계 고민_연애" in spreads_data.get(name.split(" (")[0], {}).get("categories", [])
            or "관계 고민" in spreads_data.get(name.split(" (")[0], {}).get("categories", [])
        ]
    elif main_category != "전체 스프레드":
        filtered_spreads = [
            name for name in all_spread_display_names
            if main_category in spreads_data.get(name.split(" (")[0], {}).get("categories", [])
        ]
        valid_spreads = filtered_spreads
    else:
        valid_spreads = all_spread_display_names

    for display_name in valid_spreads:
        spread_name_map[display_name] = display_name.split(" (")[0]

    spread_display_name = st.selectbox("스프레드를 선택하세요:", options=valid_spreads, help="고민의 깊이에 따라 스프레드를 선택하세요. 질문의 원인과 결과를 파악하고 싶다면 투카드 스프레드를, 깊은 통찰이 필요하다면 켈틱크로스 또는 아스타로드 스프레드를 추천합니다.")
    
    if spread_display_name:
...
if st.session_state.show_results == False:
    ...
    if st.button("프롬프트 확인 및 리딩 시작", disabled=start_button_disabled or not st.session_state.last_spread_name):
